...
5. **清理临时产物**：实验结束后，清理本地和远端 GPU 节点上的临时脚本、
   中间文件（如 `step*.sh`、`/tmp/*.sh`、容器内临时 patch 脚本等）。
   保留实验输出和日志，删除一次性执行脚本。
6. **精简文档**：实验结束并写完结论后，回过头**删除或折叠**实验设计阶段的过渡性内容
   （Phase 0 确认、假设推演、预期结果等），只保留：
   - 可复现要点（环境、命令、关键参数）
   - 核心数据与结果表格
   - 结论与 Next Step
   
   目的：避免已完结实验的文档被冗长的设计草稿淹没，让读者直接看到"做了什么、结果如何、下一步"。
   总览表中的一行摘要 + 精简后的实验段落，应足以让人快速理解。


### Phase 4: 实验完成后归档与冻结（Archive & Freeze）

实验写完结论、清理临时产物（Phase 3）后，相关设计/实验文档进入归档状态。
核心原则：**区分"活文档"与"历史档案"，归档而非删除。**

为什么不删：代码 + git diff 只能说明"现在是什么"，说不清"为什么这么设计、
试过哪些方案、为什么否决"。设计文档和实验记录的唯一价值就是这份决策依据，
也是事后最难重建的东西——删掉等于丢进 git 考古坑，理论上还在，实际没人去翻。

判断时机：当一篇实验/设计文档对应的功能**已落地并验证、且不再有后续迭代**时，就归档。

归档动作：

1. **加冻结状态行**：在已完成的设计文档 / 实验记录顶部加一行状态，例如
   `> Status: as-built (frozen YYYY-MM)。最新设计以 <活文档路径> 为准。`
   冻结后该文件只读，不再随后续迭代更新。
2. **单一真相来源**：活文档（对外的设计/架构文档）持续演进，永远反映最新状态；
   冻结的 featureN / expN 文档只描述"当初怎么来的"，避免两份文档对同一设计
   各执一词（双源漂移）。耐久的设计 rationale 应上提到活文档，feature 文档
   退化为"链接活文档 + 保留实验证据"。
3. **可选物理归档**：把已冻结文档移到 `docs/archive/`（或同义目录），
   让活跃目录只剩仍在演进的文档，一眼看出"哪些还在动"。
4. **内部序号留在内部**：partN / featureN 这类内部序号可留在内部档案里，
   但不要泄漏到对外（public）文档。
...
### Phase 6: 无人值守长实验迭代（Autonomous Long-Run Mode）

对于跨越多轮、需要长时间运行（>1h 或 overnight）的实验，可以在**无人反馈**的情况下
让 agent 自主迭代。但必须遵守下列规则，防止陷入局部最优路径而忘记真正要解决的问题。

#### 6.1 自主决策规则：默认选最推荐方案

当一轮实验结束、需要规划下一步时，如果 LLM 给出多个候选方案：

1. **直接选择最推荐的方案**继续执行，不要停下来等人决策
2. 在文档中明确写出：
   - 候选方案有哪些（A / B / C ...）
   - 选中哪一个，**为什么**（一句话理由）
   - 被放弃的方案是否值得后续回头尝试（标记 backlog）
3. 不要在"等用户确认"上消耗轮次 —— 只有在以下情况才停下来等人：
   - 实验需要消耗显著资源（GPU 占用 > 1 节点 × 24h、或费用敏感）
   - 涉及破坏性操作（删除数据、覆盖 checkpoint、强推生产分支）
   - 三轮以上仍无进展，需要重新审视问题（见 6.2）

#### 6.2 防局部路径：每轮回到文档开头

每一轮实验**结束后**、规划下一轮**之前**，必须执行下面这个动作：

> **回到实验文档开头，重读"最初要解决的问题"和总览表，问自己 3 个问题：**
>
> 1. 我现在在做的事，是否还在解决**最初的问题**？还是已经在解决"为了解决最初问题而引入的子问题的子问题"？
> 2. 已经迭代了多少轮？如果 ≥ 3 轮仍没有逼近原始目标，是不是该跳出当前路径？
> 3. 是否有更直接的方案被当时的"最推荐"挡掉了？现在重新评估，那条路是否反而更优？

如果发现已经偏离原始问题，立即在文档中开一个新段落 `## 路径回溯 / Pivot`，
记录：
- 原始目标
- 当前路径走到了哪里
- 为什么决定 pivot（或为什么决定继续）
- 新方向的第一步实验

**不要假装没有偏离**。被诚实记录的 pivot 比"看起来线性推进"的实验更有价值。

#### 6.3 自主模式下的硬性中止条件（Stop Conditions）

满足任一条件，**强制停止自动迭代**，等待人介入：

- 连续 3 轮指标无改善（或在 noise 范围内波动）
- 连续 2 轮出现相同的环境/依赖错误，且修复未生效
- 资源使用超出预算（时间 / GPU 小时 / 磁盘）
- 发现 Phase 0 假设可能不成立（observation 不完备、问题本身不可解等）

中止时在文档中写一个 `## Auto-Stop Report`，列出已尝试方案、当前卡点、建议人介入的方向。
...
7. **自主但不迷路**：长实验可不等人反馈、默认选最推荐方案继续，但每轮结束后必须回到文档开头重读原始问题，防止陷入"为解决子问题的子问题"的局部路径
